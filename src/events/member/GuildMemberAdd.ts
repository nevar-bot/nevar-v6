...
class GuildMemberAddEvent extends BaseEvent {
	...
	public async run(addedGuildMember: GuildMember): Promise<void> {
		if (
			!addedGuildMember ||
			!addedGuildMember.id ||
			!addedGuildMember.guild ||
			!addedGuildMember.guild.available ||
			addedGuildMember.pending
		)
			return;
		...
		const embedText: string =
			'### ' +
			this.emote('member_create') +
			' Mitglied hat den Server betreten\n\n' +
			'-# ' +
			this.emote('calendar') +
			' **Erstellt am**: ' +
			this.formatUtils.discordTimestamp(addedGuildMember.user.createdTimestamp, 'f') +
			'\n' +
			'-# ' +
			this.emote('reminder') +
			' **Erstellt vor**: ' +
			this.formatUtils.discordTimestamp(addedGuildMember.user.createdTimestamp, 'R');

		const memberLogEmbed = this.clientUtils.createEmbed(embedText, null, 'success').setAuthor({
			name: addedGuildMember.user.username,
			iconURL: addedGuildMember.displayAvatarURL(),
		});
		...
		if (guildData.settings.welcome.autoroles.length > 0) {
			...
			for (let roleId of guildData.settings.welcome.autoroles) {
				const role: Role | void =
					guild.roles.cache.get(roleId) ||
					(await guild.roles.fetch(roleId).catch(() => {}));
		...
		if (guildData.settings.welcome.enabled) {
			...
			if (!guildMessage) {
				this.commandUtils
					.commandMention('welcome', 'nachricht')
					.then(async (mention: string) => {
						const embedText: string =
							'### ' +
							this.emote('error') +
							' Willkommensnachricht nicht gefunden\n\n' +
							'-# ' +
							this.emote('arrow_right') +
							' **Fehler**: Es wurde keine Willkommensnachricht gesetzt. Bitte setze die Willkommensnachricht mit: ' +
							mention;
			...
			const welcomeMessage: string = this.parseWelcomeMessage(
				guildMessage,
				addedGuildMember,
				guild
			);
			...
			if (!channel) {
				this.commandUtils
					.commandMention('welcome', 'kanal')
					.then(async (mention: string) => {
						const embedText: string =
							'### ' +
							this.emote('error') +
							' Willkommenskanal nicht gefunden\n\n' +
							'-# ' +
							this.emote('arrow_right') +
							' **Fehler**: Der Willkommenskanal wurde nicht gefunden. Bitte setze den Willkommenskanal mit: ' +
							mention;
			...
			if (!guildData.settings.welcome.color) {
				this.commandUtils
					.commandMention('welcome', 'farbe')
					.then(async (mention: string) => {
						const embedText: string =
							'### ' +
							this.emote('error') +
							' Willkommensfarbe nicht gefunden\n\n' +
							'-# ' +
							this.emote('arrow_right') +
							' **Fehler**: Die Willkommensfarbe wurde nicht gefunden. Bitte setze die Willkommensfarbe mit: ' +
							mention;
	...
	private parseWelcomeMessage(
		message: string,
		addedGuildMember: GuildMember,
		guild: any
	): string {
		return message
