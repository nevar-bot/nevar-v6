	...
	public async run(message: Message): Promise<void> {
		...
		if (guildData.settings?.autodelete.length > 0) {
			for (let autodelete of guildData.settings.autodelete) {
				...
				if (
					(channelId !== channel.id && channel.type !== 11) ||
					(channel.type === 11 &&
		...
		const memberData: any = await this.databaseUtils.findOrCreateMember(
			member.user.id,
			guild.id
		);
		const userData: any = await this.databaseUtils.findOrCreateUser(member.user.id);

		const botMentionPattern = new RegExp(`^<@!?${this.client.user.id}>( |)$`);
		if (
			this.regexUtils.stringMatches(botMentionPattern, message.content) &&
			!message.author.bot
		) {
			...
			const helpCommand: string = await this.commandUtils.commandMention('help');
			...
			const greetingText: string =
				...
				greetings[greeting] +
				'\n\n' +
				this.emote('logo_icon') +
				' Ich bin ' +
				this.client.user.toString() +
				' und **unterstütze derzeit über ' +
				flooredGuildCount +
				' Server** bei Moderation und Verwaltung.\n\n' +
				'-# ' +
				this.emote('question_mark') +
				' Mit ' +
				helpCommand +
				'erhältst du eine **Übersicht meiner verfügbaren Befehle**.';

			const greetingEmbed: EmbedBuilder = this.clientUtils
				.createEmbed(greetingText, null, 'normal')
				.setThumbnail(this.client.user.displayAvatarURL());

			const inviteButton: ButtonBuilder = this.componentUtils.createButton(
				null,
				'Einladen',
				ButtonStyle.Link,
				this.emote('logo_icon'),
				false,
				this.clientUtils.createInvite()
			);
			const supportButton: ButtonBuilder = this.componentUtils.createButton(
				null,
				'Support',
				ButtonStyle.Link,
				this.emote('discord'),
				false,
				config.get('support.invitation')
			);
			const buttonActionRow = this.componentUtils.createActionRow(
				inviteButton,
				supportButton
			);
			...
			if (this.regexUtils.stringMatches(botMentionPattern, botMention)) {
				...
				if (
					chatCommand &&
					(chatCommand.restrictions.staffOnly || chatCommand.restrictions.ownerOnly)
				) {
					if (
						!userData.staff.status &&
						!config.get('client.owner_ids').includes(member.user.id)
					)
						return;
					if (
						chatCommand.general.category === 'owner' &&
						userData.staff.role !== 'head-staff' &&
						!config.get('client.owner_ids').includes(member.user.id)
					)
						return;
		...
		if (guildData.settings?.levelsystem.enabled) {
			...
			const emojiMentionRegex = /<a?:\w+:(\d+)>/g;
			...
			const minLengthForXp = 3;
			const minLengthForMaxXp = 50;
			const maxPossibleXp = 35;
			...
			if (guildData.settings?.levelsystem.exclude.channelIds.length > 0) {
				if (guildData.settings.levelsystem.exclude.channelIds.includes(channel.id))
					gainedXp = 0;
			}
			...
			if (guildData.settings?.levelsystem.exclude.roleIds.length > 0) {
				...
				if (
					guildData.settings.levelsystem.exclude.roleIds.some((roleId: string): boolean =>
			...
			if (guildData.settings?.levelsystem.doubleXP.length > 0) {
				const memberRoles = member.roles.cache.map((role: Role) => role.id);
				if (
					guildData.settings.levelsystem.doubleXP.some((roleId: string): boolean =>
						memberRoles.includes(roleId)
					)
				)
					gainedXp *= 2;
			...
			const userLeveledUp: boolean = await levelManagerInstance.appendXp(
				member.user.id,
				guild.id,
				gainedXp
			);
			if (userLeveledUp) {
				...
				const levelUpChannel: any = guild.channels.cache.get(
					guildData.settings.levelsystem.levelUpMessage.channelId || channel.id
				);
				...
				const parsedLevelMessage: string = parseLevelMessage(
					guildData.settings.levelsystem.levelUpMessage.message
				);
